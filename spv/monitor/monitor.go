...
func collectMetrics() (Metrics, error) {
    ...
    // Fetch transaction count and details
    transactionsStr, err := fetchEndpoint("/getTransactions")
    if err != nil {
        return metrics, err
    }
    transactionCount := 0
    var transactionsOutput strings.Builder

    transactionsParts := strings.Split(transactionsStr, "----------------------")
    for _, part := range transactionsParts {
        ...
        if txid != "" && amount != "" {
            transactionsOutput.WriteString(fmt.Sprintf("txid: %s\namount: %s\n", txid, amount))
            transactionCount++
        }
    }
    metrics.Transactions = transactionsOutput.String()
    metrics.TransactionCount = fmt.Sprintf("%d", transactionCount)

    // Fetch unspent UTXO count and details
    utxosStr, err := fetchEndpoint("/getUTXOs")
    if err != nil {
        return metrics, err
    }
    unspentCount := 0
    var utxosOutput strings.Builder

    utxosParts := strings.Split(utxosStr, "----------------------")
    for _, part := range utxosParts {
        part = strings.TrimSpace(part)
        if part == "" {
            continue
        }
        ...
        if txid != "" && amount != "" {
            utxosOutput.WriteString(fmt.Sprintf("txid: %s\namount: %s\n", txid, amount))
            unspentCount++
        }
    }
    metrics.UTXOs = utxosOutput.String()
