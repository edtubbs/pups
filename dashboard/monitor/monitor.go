...
func fetchEndpoint(endpoint string) (string, error) {
    ...
    if err != nil { return "", fmt.Errorf("error sending request to %s: %w", endpoint, err) }
    ...
    if err != nil { return "", fmt.Errorf("error reading response body for %s: %w", endpoint, err) }
...
func collectMetrics() (Metrics, map[string]string, map[string]string, error) {
    ...
    if err != nil { return metrics, nil, nil, err }
    metrics.Chaintip = parseSimpleMetric(chaintipStr, "Chain tip: ")

    // Balance
    balanceStr, err := fetchEndpoint("/getBalance")
    if err != nil { return metrics, nil, nil, err }
    metrics.RawBalance = parseSimpleMetric(balanceStr, "Wallet balance: ")
    metrics.Balance = fmt.Sprintf("%sÐ", metrics.RawBalance)
...
func parseListMetric(input, prefix string) string {
    ...
    if len(items) > 0 { return strings.Join(items, "\n") }
...
func submitMetrics(metrics Metrics, stats map[string]string, chain map[string]string) {
    ...
    // Build payload (all values as strings for consistency)
    jsonData := map[string]interface{}{
        "chaintip":          map[string]interface{}{"value": metrics.Chaintip},
        "balance":           map[string]interface{}{"value": mustParseFloat(metrics.RawBalance)},
        "addresses":         map[string]interface{}{"value": metrics.Addresses},
        "transaction_count": map[string]interface{}{"value": mustParseFloat(metrics.TransactionCount)},
        "unspent_count":     map[string]interface{}{"value": mustParseFloat(metrics.UnspentCount)},
...
func mustParseFloat(s string) float64 {
    f, err := strconv.ParseFloat(s, 64)
    if err != nil {
        log.Printf("Error parsing float: %v for value %s", err, s)
        return 0
    }
    return f
}
